...
import { MutationCreateSessionArgs, MutationRefreshSessionArgs, Session as AgoraSession } from "../generated/graphql-schema";
...
export async function refreshAccessTokenIfNeeded(request: Request): Promise<void> {
  ...
  if (me === null) {
    ...
    const refreshedSession = await refreshAgoraSession({
      username: username,
...
export async function createAgoraSession(args: MutationCreateSessionArgs) {
  const request = gql`
...
export async function refreshAgoraSession(args: MutationRefreshSessionArgs): Promise<AgoraSession> {
  const request = gql`
...
const { getSession, commitSession, destroySession } = createCookieSessionStorage({
  cookie: {
    ...
    secrets: ["CookieSecretsAreNotNecessaryForOurPurposesButHereYouAre"],
    sameSite: "lax",
    path: "/",
    maxAge: 60 * 60 * 24 * 30,
    httpOnly: true,
