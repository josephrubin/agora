...
const BUCKET_NAME = process.env.AGORA_CLIENT_MEDIA_UPLOAD_BUCKET;
const GRAPHQL_ENDPOINT_URL = process.env.AGORA_GRAPHQL_URL;
...
// as possible while still leaving enough tie to upload a large file.
...
const lambdaHandler: LambdaHandlerType = async ({ body }) => {
  ...
  const signedUrlKey = `media/${userId}/${bodyData.pieceId}/${uuidv4()}.ogg`;

  const putMediaObjectCommand = new PutObjectCommand({
    Bucket: BUCKET_NAME,
    Key: signedUrlKey,
  ...
  try {
    const signedUrl = await getSignedUrl(s3Client, putMediaObjectCommand, {
